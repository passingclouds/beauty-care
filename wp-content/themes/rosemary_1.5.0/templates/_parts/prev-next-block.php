...
if (!$post_data['post_protected']) {
	...
	if (!empty($post_options['dedicated'])) {
		...
		$page_style = rosemary_get_custom_option('single_style');
		?>
		<div class="post_thumb post_nav"<?php echo ($page_style=='single-portfolio-fullscreen' ? ' style="background-image:url('.esc_url($post_data['post_attachment']).');"' : ''); ?>>
			...
			$args = rosemary_query_add_posts_and_cats($args, '', $post_data['post_type'],
			...
			if ( $prev ) {
				...
				$desc = rosemary_strshort($prev->post_title, 30);
				?>
			...
			if ( $next ) {
				...
				$desc = rosemary_strshort($next->post_title, 30);
				?>
