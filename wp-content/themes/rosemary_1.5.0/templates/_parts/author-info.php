<?php
//===================================== Post author info =====================================
if (rosemary_get_custom_option("show_post_author") == 'yes') {
	$post_author_name = $post_author_socials = '';
	$show_post_author_socials = false;
	if ($post_data['post_type']=='post') {
		$post_author_title = esc_html__('About', 'rosemary');
		...
		$post_author_avatar = get_avatar($post_author_email, 75*min(2, max(1, rosemary_get_theme_option("retina_ready"))));
		$post_author_descr = rosemary_do_shortcode(nl2br(get_the_author_meta('description', $post_data['post_author_id'])));
		if ($show_post_author_socials) $post_author_socials = rosemary_show_user_socials(array('author_id'=>$post_data['post_author_id'], 'size'=>'tiny', 'echo' => false));
	}
