...
if ( !function_exists( 'rosemary_template_services_1_theme_setup' ) ) {
	add_action( 'rosemary_action_before_init_theme', 'rosemary_template_services_1_theme_setup', 1 );
	function rosemary_template_services_1_theme_setup() {
		rosemary_add_template(array(
			'layout' => 'services-1',
			'template' => 'services-1',
			'mode'   => 'services',
			'title'  => esc_html__('Services /Style 1/', 'rosemary'),
...
// Template output
if ( !function_exists( 'rosemary_template_services_1_output' ) ) {
	function rosemary_template_services_1_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			<div<?php echo ($post_options['tag_id'] ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''); ?>
				...
				<span class="sc_title_divider_after"><?php echo esc_attr('//');?></span>
					<?php
					if ($show_title) {
						if ((!isset($post_options['links']) || $post_options['links']) && !empty($post_data['post_link'])) {
