...
if ( !function_exists( 'rosemary_template_services_2_theme_setup' ) ) {
	add_action( 'rosemary_action_before_init_theme', 'rosemary_template_services_2_theme_setup', 1 );
	function rosemary_template_services_2_theme_setup() {
		rosemary_add_template(array(
			'layout' => 'services-2',
			'template' => 'services-2',
			'mode'   => 'services',
			'need_columns' => true,
			'title'  => esc_html__('Services /Style 2/', 'rosemary'),
...
// Template output
if ( !function_exists( 'rosemary_template_services_2_output' ) ) {
	function rosemary_template_services_2_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			<div<?php echo ($post_options['tag_id'] ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''); ?>
				...
				if ($post_data['post_icon'] && $post_options['tag_type']=='icons') {
					$html = rosemary_do_shortcode('[trx_icon icon="'.esc_attr($post_data['post_icon']).'" shape="round"]');
				...
				<div class="sc_services_item_content">
					...
					<div class="sc_services_item_description">
						...
						if ($post_data['post_protected']) {
							...
							if (!empty($post_data['post_link']) && !rosemary_param_is_off($post_options['readmore'])) {
