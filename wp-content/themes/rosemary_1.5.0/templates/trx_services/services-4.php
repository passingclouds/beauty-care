...
if ( !function_exists( 'rosemary_template_services_4_theme_setup' ) ) {
	add_action( 'rosemary_action_before_init_theme', 'rosemary_template_services_4_theme_setup', 1 );
	function rosemary_template_services_4_theme_setup() {
		rosemary_add_template(array(
			'layout' => 'services-4',
			'template' => 'services-4',
			'mode'   => 'services',
			'need_columns' => true,
			'title'  => esc_html__('Services /Style 4/', 'rosemary')
		));
	}
}

// Template output
if ( !function_exists( 'rosemary_template_services_4_output' ) ) {
	function rosemary_template_services_4_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			<div<?php echo ($post_options['tag_id'] ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''); ?>
				...
				echo trim(rosemary_do_shortcode('[trx_icon icon="'.esc_attr($post_data['post_icon']).'" shape="round"]'));
