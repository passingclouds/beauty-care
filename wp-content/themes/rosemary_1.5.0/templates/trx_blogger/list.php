...
if ( !function_exists( 'rosemary_template_list_theme_setup' ) ) {
	add_action( 'rosemary_action_before_init_theme', 'rosemary_template_list_theme_setup', 1 );
	function rosemary_template_list_theme_setup() {
		rosemary_add_template(array(
			'layout' => 'list',
			'mode'   => 'blogger',
			'need_columns' => true,
			'title'  => esc_html__('Blogger layout: List', 'rosemary')
			));
	}
}

// Template output
if ( !function_exists( 'rosemary_template_list_output' ) ) {
	function rosemary_template_list_output($post_options, $post_data) {
