...
if ( !function_exists( 'rosemary_template_events_1_theme_setup' ) ) {
	add_action( 'rosemary_action_before_init_theme', 'rosemary_template_events_1_theme_setup', 1 );
	function rosemary_template_events_1_theme_setup() {
		rosemary_add_template(array(
			'layout' => 'events-1',
			'template' => 'events-1',
			'mode'   => 'events',
			'title'  => esc_html__('Events /Style 1/', 'rosemary'),
...
// Template output
if ( !function_exists( 'rosemary_template_events_1_output' ) ) {
	function rosemary_template_events_1_output($post_options, $post_data) {
		$show_title = true;
							...
							if ($post_data['post_excerpt']) {
								echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) 
										...
										: '<p>'.trim(rosemary_strshort($post_data['post_excerpt'], isset($post_options['descr'])
																										? $post_options['descr'] 
																										: rosemary_get_custom_option('post_excerpt_maxlength_masonry'))).'</p>';
							}
