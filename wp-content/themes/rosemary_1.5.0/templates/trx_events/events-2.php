...
if ( !function_exists( 'rosemary_template_events_2_theme_setup' ) ) {
	add_action( 'rosemary_action_before_init_theme', 'rosemary_template_events_2_theme_setup', 1 );
	function rosemary_template_events_2_theme_setup() {
		rosemary_add_template(array(
			'layout' => 'events-2',
			'template' => 'events-2',
			'mode'   => 'events',
			'title'  => esc_html__('Events /Style 2/', 'rosemary')
		));
	}
}

// Template output
if ( !function_exists( 'rosemary_template_events_2_output' ) ) {
	function rosemary_template_events_2_output($post_options, $post_data) {
		$show_title = true;
		...
		<div<?php echo ($post_options['tag_id'] ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''); ?>
			...
			?><span class="sc_events_item_time"><?php
				echo (trim($start_date[1]) ? $start_date[1] : esc_html__('Whole day', 'rosemary'))
