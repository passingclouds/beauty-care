...
if ( !function_exists( 'rosemary_template_testimonials_2_theme_setup' ) ) {
	add_action( 'rosemary_action_before_init_theme', 'rosemary_template_testimonials_2_theme_setup', 1 );
	function rosemary_template_testimonials_2_theme_setup() {
		rosemary_add_template(array(
			...
			'title'  => esc_html__('Testimonials /Style 2/', 'rosemary')
		));
	}
}

// Template output
if ( !function_exists( 'rosemary_template_testimonials_2_output' ) ) {
	function rosemary_template_testimonials_2_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			<div<?php echo ($post_options['tag_id'] ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''); ?> class="sc_testimonial_item<?php echo (!empty($post_options['tag_class']) ? ' '.esc_attr($post_options['tag_class']) : ''); ?>"<?php echo ($post_options['tag_css'] ? ' style="'.esc_attr($post_options['tag_css']).'"' : '');?>>
				...
				<div class="sc_title_divider_after"><?php echo esc_attr('//');?></div>
