<?php
/**
 * RoseMary Framework: social networks
 *
 * @package	rosemary
 * @since	rosemary 1.0
 */
...
// Theme init
if (!function_exists('rosemary_socials_theme_setup')) {
	add_action( 'rosemary_action_before_init_theme', 'rosemary_socials_theme_setup' );
	function rosemary_socials_theme_setup() {

		if ( !is_admin() ) {
			// Add og:image meta tag for facebook
			add_action( 'wp_head', 					'rosemary_facebook_og_tags', 5 );
		}
...
$ROSEMARY_GLOBALS['share_links'] = array(
	...
	'postila' =>		'http://postila.ru/publish/?url={link}&agregator=rosemary',
...
if (!function_exists('rosemary_add_share_link')) {
	function rosemary_add_share_link($soc, $url='') {
		if (!is_array($soc)) $soc = array($soc => $url);
		global $ROSEMARY_GLOBALS;
		$ROSEMARY_GLOBALS['share_links'] = array_merge( $ROSEMARY_GLOBALS['share_links'], $soc );
	}
}

// Return (and show) share social links
if (!function_exists('rosemary_show_share_links')) {
	function rosemary_show_share_links($args) {
		if ( rosemary_get_custom_option('show_share')=='hide' ) return '';
		...
		$args = array_merge(array(
			...
			'style' => rosemary_get_theme_setting('socials_type')=='images' ? 'bg' : 'icons',	// style for show icons: icons|images|bg
			'type' => 'block',					// share block type: list|block|drop
			'popup' => true,					// open share url in new window or in popup window
			'counters' => rosemary_get_custom_option('show_share_counters')=='yes',	// show share counters
			'direction' => rosemary_get_custom_option('show_share'),				// share block direction
			'caption' => rosemary_get_custom_option('share_caption'),				// share block caption
			'share' => rosemary_get_theme_option('share_buttons'),					// list of allowed socials
		...
		if (is_array($args['share']) && count($args['share']) > 0) {
			foreach ($args['share'] as $soc) {
				$icon = $args['style']=='icons' || rosemary_strpos($soc['icon'], $upload_url)!==false ? $soc['icon'] : rosemary_get_socials_url(basename($soc['icon']));
				if ($args['style'] == 'icons') {
				...
				$url = empty($soc['url']) && !empty($ROSEMARY_GLOBALS['share_links'][$sn]) ? $ROSEMARY_GLOBALS['share_links'][$sn] : $soc['url'];
				...
				$output .= '<div class="sc_socials_item">'
					...
					. ($args['type']=='drop' ? '<i>' . trim(rosemary_strtoproper($sn)) . '</i>' : '')
					. '</div>';
			}
		}
		$output .= '</div>';
...
if (!function_exists('rosemary_prepare_socials')) {
	function rosemary_prepare_socials($list, $style='') {
		if (empty($style)) $style = rosemary_get_theme_setting('socials_type')=='images' ? 'bg' : 'icons';
		$output = '';
		$upload_info = wp_upload_dir();
		$upload_url = $upload_info['baseurl'];
		if (is_array($list) && count($list) > 0) {
			foreach ($list as $soc) {
				...
				$icon = $style=='icons' || rosemary_strpos($soc['icon'], $upload_url)!==false ? $soc['icon'] : rosemary_get_socials_url(basename($soc['icon']));
				if ($style == 'icons') {
					$parts = explode('-', $soc['icon'], 2);
					$sn = isset($parts[1]) ? $parts[1] : $parts[0];
				} else {
...
if (!function_exists('rosemary_get_twitter_data')) {
	function rosemary_get_twitter_data($cfg) {
		return function_exists('rosemary_twitter_acquire_data')
				? rosemary_twitter_acquire_data(array(
						'mode'            => 'user_timeline',
...
if (!function_exists('rosemary_prepare_twitter_text')) {
	function rosemary_prepare_twitter_text($tweet) {
		$text = $tweet['text'];
...
if (!function_exists('rosemary_get_twitter_followers')) {
	function rosemary_get_twitter_followers($cfg) {
		$data = rosemary_get_twitter_data($cfg);
...
if (!function_exists('rosemary_get_facebook_likes')) {
	function rosemary_get_facebook_likes($account) {
		$fb = get_transient("facebooklikes");
		if ($fb !== false) return $fb;
		$fb = '?';
		$url = esc_url(rosemary_get_protocol().'http://graph.facebook.com/'.($account));
		$headers = get_headers($url);
		if (rosemary_strpos($headers[0], '200')) {
			$json = rosemary_fgc($url);
...
if (!function_exists('rosemary_facebook_og_tags')) {
	function rosemary_facebook_og_tags() {
		global $post;
		if ( !is_singular() || rosemary_get_global('blog_streampage')) return;
...
if (!function_exists('rosemary_get_feedburner_counter')) {
	function rosemary_get_feedburner_counter($account) {
		$rss = get_transient("feedburnercounter");
		if ($rss !== false) return $rss;
		$rss = '?';
		$url = esc_url(rosemary_get_protocol().'://feedburner.google.com/api/awareness/1.0/GetFeedData?uri='.($account));
		$headers = get_headers($url);
		if (rosemary_strpos($headers[0], '200')) {
			$xml = rosemary_fgc($url);
