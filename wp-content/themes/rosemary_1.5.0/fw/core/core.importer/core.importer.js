// RoseMary Importer script

jQuery(document).ready(function(){
	"use strict";
	
	// Show/hide fetch attachment
	jQuery('#trx_importer_form #import_posts').change(function() {
	...
	// Start import
	jQuery('.trx_importer_section .trx_buttons input[type="button"]').click(function() {
		"use strict";
		...
		rosemary_importer_do_action(steps, 0);
	});
});

// Call specified action (step)
function rosemary_importer_do_action(steps, idx) {
	"use strict";
	...
	// AJAX query params
	var data = {
		ajax_nonce: ROSEMARY_GLOBALS['ajax_nonce'],
		action: 'rosemary_importer_start_import',
	...
	jQuery.post(ROSEMARY_GLOBALS['ajax_url'], data, function(response) {
		...
		try {
			...
			rez = {error: ROSEMARY_GLOBALS['ajax_error']}
		}
		if (rez.error === '') {
			...
			// Do next action
			if (idx < steps.length) {
				rosemary_importer_do_action(steps, idx);
			} else {
