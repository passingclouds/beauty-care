 ...
 */
add_action( 'widgets_init', 'rosemary_widget_advert_load' );

/**
 * Register our widget.
 */
function rosemary_widget_advert_load() {
	register_widget( 'rosemary_widget_advert' );
}

/**
 * Twitter Widget class.
 */
class rosemary_widget_advert extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'widget_advert', 'description' => esc_html__('Advertisement block', 'rosemary') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 200, 'height' => 250, 'id_base' => 'rosemary_widget_advert' );

		/* Create the widget. */
		parent::__construct( 'rosemary_widget_advert', esc_html__('RoseMary - Advertisement block', 'rosemary'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		<div class="widget_advert_inner">
			...
			if ($advert_code!='') {
				echo force_balance_tags(rosemary_substitute_all($advert_code));
			}
			?>
		</div>
		<?php
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => '', 'description' => esc_html__('Advertisement block', 'rosemary') );
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'advert_image' )); ?>"><?php esc_html_e('Image source URL:<br />(leave empty if you paste advert code)', 'rosemary'); ?></label>
            ...
            <?php
			echo trim(rosemary_show_custom_field($this->get_field_id( 'advert_media' ), array('type'=>'mediamanager', 'media_field_id'=>$this->get_field_id( 'advert_image' )), null));
			if ($advert_image) {
			?>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'advert_link' )); ?>"><?php esc_html_e('Image link URL:<br />(leave empty if you paste advert code)', 'rosemary'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'advert_code' )); ?>"><?php esc_html_e('or paste Advert Widget HTML Code:', 'rosemary'); ?></label>
