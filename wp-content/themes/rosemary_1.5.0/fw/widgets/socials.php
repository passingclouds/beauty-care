 ...
 */
add_action( 'widgets_init', 'rosemary_widget_socials_load' );

/**
 * Register our widget.
 */
function rosemary_widget_socials_load() {
	register_widget( 'rosemary_widget_socials' );
}

/**
 * flickr Widget class.
 */
class rosemary_widget_socials extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'widget_socials', 'description' => esc_html__('Show site logo and social links', 'rosemary') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 200, 'height' => 250, 'id_base' => 'rosemary_widget_socials' );

		/* Create the widget. */
		parent::__construct( 'rosemary_widget_socials', esc_html__('RoseMary - Show logo and social links', 'rosemary'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		$text = isset($instance['text']) ? rosemary_do_shortcode($instance['text']) : '';
		...
		<div class="widget_inner">
            <?php
				if ($show_logo) {
					...
					if ($logo_image=='')	$logo_image = $ROSEMARY_GLOBALS['logo'];
					if ($logo_text=='')		$logo_text = $ROSEMARY_GLOBALS['logo_text'];
					if ($logo_slogan=='')	$logo_slogan = $ROSEMARY_GLOBALS['logo_slogan'];
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => '', 'logo_image'=>'', 'logo_text'=>'', 'show_logo' => '1', 'show_icons' => '1', 'text'=>'', 'description' => esc_html__('Show logo and social icons', 'rosemary') );
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'text' )); ?>"><?php esc_html_e('Description:', 'rosemary'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'logo_image' )); ?>"><?php esc_html_e('Logo image:<br />(if empty - use logo from Theme Options)', 'rosemary'); ?></label>
            ...
            <?php
			echo trim(rosemary_show_custom_field($this->get_field_id( 'logo_media' ), array('type'=>'mediamanager', 'media_field_id'=>$this->get_field_id( 'logo_image' )), null));
			if ($logo_image) {
			?>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'logo_text' )); ?>"><?php esc_html_e('Logo text:', 'rosemary'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_logo')); ?>_1"><?php esc_html_e('Show logo:', 'rosemary'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_logo')); ?>_1"><?php esc_html_e('Show', 'rosemary'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_logo')); ?>_0"><?php esc_html_e('Hide', 'rosemary'); ?></label>
		</p>
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_icons')); ?>_1"><?php esc_html_e('Show social icons:', 'rosemary'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_icons')); ?>_1"><?php esc_html_e('Show', 'rosemary'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_icons')); ?>_0"><?php esc_html_e('Hide', 'rosemary'); ?></label>
		</p>
